...
| **[!UICONTROL Estimated Pickup Lead Time]**</br>`Extension Attribute: pickup_lead_time` | Nombre de minutes nécessaires avant la réception d’une commande, le moment où elle est sélectionnée et le moment où elle est prête à être sélectionnée. </br></br>Ces informations sont utilisées pour l’affichage d’une estimation des heures pour la récupération de la commande auprès des clients sur le site web.</br></br> La définition de cette option remplace la configuration globale pour **Délai d’avance estimé pour la collecte** configuré pour le **Méthode de diffusion** dans le **Collecte en magasin** configuration. | Global | Non |
