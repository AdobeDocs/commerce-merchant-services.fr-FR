...
>[!NOTE]
>
>Pour éviter toute confusion, n’utilisez pas un service Commerce spécifique comme nom de projet, par exemple *Recherche en direct*, *Recommendations de produit* ou *Connexion aux données*.  À moins que votre licence n’ait été configurée pour plusieurs projets SaaS, vous pouvez utiliser le même projet SaaS pour plusieurs services.
...
Pour connecter votre instance Adobe Commerce au Adobe Experience Platform, connectez-vous à votre compte Adobe à l’aide de votre Adobe ID. Une fois que vous êtes connecté, l’organisation IMS associée à votre compte d’Adobe s’affiche dans cette section.
