...
Pour créer une règle, la première étape consiste à utiliser l’éditeur de règles pour définir la ou les conditions dans le texte de requête de l’acheteur qui déclenchent le ou les événements associés. Renseignez ensuite les détails de la règle, testez les résultats et publiez la règle.

## Étape 1 : Ajouter une règle

1. Dans Admin, accédez à **Marketing** > SEO &amp; Search > **Recherche en direct**.
1. Définissez la variable **Portée** pour identifier la variable [vue de magasin](https://docs.magento.com/user-guide/configuration/scope.html) lorsque la règle s’applique.
...
## Étape 2 : Décrire la ou les conditions

Les conditions sont les conditions requises pour déclencher un événement. Une règle peut comporter jusqu’à dix conditions et vingt-cinq événements.
...
1. Sous *Créer votre règle*, sélectionnez la variable **Condition** et suivez les instructions pour remplir l’instruction .
   ...
   * La requête de recherche contient : entrez la chaîne de texte qui doit se trouver dans la requête de l’acheteur. Le paramètre Correspondance détermine le degré auquel la requête de l’acheteur correspond au catalogue. Options :<br /> N’importe lequel : toute partie du texte de requête de l’acheteur peut correspondre à la condition.<br />Tous : toutes les requêtes de l’acheteur doivent correspondre à la condition .
...
1. Pour créer une règle simple avec une condition, accédez à l’étape 3 : [Ajouter un ou plusieurs événements](#events).

### Conditions multiples

1. Pour créer une règle avec plusieurs conditions, cliquez sur **Ajouter une condition**.
Une règle peut comporter jusqu’à 10 conditions. L’opérateur logique qui rejoint deux conditions est basé sur la condition actuelle *Correspondance* . Par défaut, *Correspondance* is `All` et l’opérateur logique est `AND`.
...
1. Pour plusieurs événements, choisissez les autres événements à déclencher lorsque la ou les conditions sont remplies.
...
1. Saisissez le **Date de début** et **Date de fin** lorsque la règle sera principale ou sélectionnez les dates du calendrier.
...
|--- |--- |
| La requête de recherche contient | Caractère ou chaîne de texte inclus dans la requête de l’acheteur. La requête du nouvel acheteur ne doit correspondre qu’à un seul caractère pour remplir cette condition. |
...
|--- |--- |
| Amplifier | Déplace un SKU ou une plage de SKU plus haut dans les résultats de recherche. Chacun d’eux est marqué d’un badge d’aperçu &quot;rechargé&quot; dans les résultats de recherche de test. |
| Bury | Déplace un SKU ou une plage de SKU vers le bas dans les résultats de recherche. Chacun est marqué d’un badge d’aperçu &quot;enterré&quot; dans les résultats de recherche de test. |
| Epinglage d’un produit | Associe un seul SKU à une position spécifique dans les résultats de recherche. Le produit est marqué d’un badge d’aperçu &quot;épinglé&quot; dans les résultats de recherche de test. |
